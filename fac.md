# ❓ FAQ (Perguntas Frequentes)

### Inscrições e Auxílios

* **O evento oferece auxílio para transporte, hospedagem ou alimentação?** Não. O evento não dispõe de recursos adicionais para fornecer ajuda de custo, deslocamento, estadia ou refeições. 
* **O que acontece se eu pagar e não houver mais vagas?** As vagas são preenchidas por ordem de confirmação do pagamento e envio do comprovante. Caso o limite seja atingido antes do processamento, o valor pago será integralmente estornado.
* **Haverá lista de espera?** Sim, se necessário.

* **Posso trocar de módulo após a inscrição?** A alteração de módulo está condicionada à existência de vagas e deve ser formalmente solicitada via `pythoncamp@cbpf.br` para análise.

### 📜 Certificados e Presença

* **Qual a regra para emissão do certificado?** A emissão do certificado está vinculada à comprovação de presença mínima de **75%** na carga horária total. 
* **Como recebo meu certificado?** O documento digital deve ser solicitado via e-mail (`pythoncamp@cbpf.br`) após o encerramento das atividades. 

### 💻 Equipamentos e Conectividade

* **Preciso levar notebook?** Sim. O uso de notebook pessoal é requisito indispensável para a participação nas atividades práticas.
* **Quais softwares devo instalar previamente?** As atividades serão baseadas preferencialmente na plataforma **Google Colab**. Eventuais instruções técnicas adicionais serão encaminhadas aos inscritos antes do início das atividades.
* **Haverá Wi-Fi no local?** Sim. O evento disponibilizará credenciais de acesso à rede sem fio aos participantes devidamente inscritos durante o período das atividades.

### 🏛️ Logística e Localização

* **O evento terá transmissão online ou formato híbrido?** Não. O evento é realizado exclusivamente de forma presencial.
* **Onde posso almoçar nas proximidades do CBPF?**
O evento não fornece almoço. Algumas opções próximas:
* **UNIRIO:** Restaurante universitário e cantinas com preços acessíveis ao lado do CBPF.
* **Shopping Rio Sul:** Grande variedade de restaurantes (5 min de caminhada).
* **Vila em frente ao CBPF:** Opções de comida caseira e quentinhas com bom custo-benefício.


Para retornar ao site principal acesse: https://github.com/pythoncampcbpf/pc2026/wiki



